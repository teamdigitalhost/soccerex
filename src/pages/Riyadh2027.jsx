  ...
  if (status === 'success') {
    return (
      <div className="flex flex-col items-center text-center" style={{ padding: '32px 8px' }}>
        ...
        <h4 className="font-heading font-bold text-white text-lg mb-2">Request received.</h4>
        <p className="font-body text-white/65 text-sm leading-relaxed">We'll be in touch as soon as registration, access, and agenda updates for Soccerex Middle East 2027 are live.</p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-3" noValidate>
      <label className="font-mono text-[10px] uppercase tracking-widest text-white/50">Full name
        <input required value={form.name} onChange={update('name')} placeholder="Your full name" className="pre-reg-input" />
      </label>
      <label className="font-mono text-[10px] uppercase tracking-widest text-white/50">Email
        <input required type="email" value={form.email} onChange={update('email')} placeholder="you@company.com" className="pre-reg-input" />
      </label>
      ...
      <button type="submit" disabled={status === 'submitting'} className="event-btn-primary mt-2 justify-center">
        {status === 'submitting' ? 'Sending, one moment' : 'Request Access'}
      </button>
    ...
    <div className="event-page theme-riyadh" style={{ background: 'var(--event-bg-dark)' }}>
      ...
      <section className="inner-hero relative overflow-hidden flex items-center">
        ...
        <div className="relative z-10 text-center w-full" style={{ padding: 'clamp(60px,7vw,110px) clamp(24px,5vw,80px) clamp(60px,8vw,100px)' }}>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            <a href="#pre-register" className="event-btn-primary"><Mail size={16} /> Request Access</a>
      ...
      <section style={{ background: 'linear-gradient(180deg, #f4f3f0 0%, #eae8e4 100%)', padding: 'clamp(140px,14vw,180px) clamp(24px,5vw,80px) clamp(60px,8vw,100px)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <h3 className="font-heading font-bold text-xl mb-3" style={{ color: '#1a1a1a' }}>WHERE THE MIDDLE EAST FOOTBALL OPPORTUNITY COMES INTO FOCUS</h3>
          <p className="font-body leading-relaxed mb-6" style={{ fontSize: '1rem', color: '#444', maxWidth: 820 }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {THEMES.map((theme) => (
              <div key={theme.title} className="flex items-start gap-4 px-6 py-5 rounded-xl" style={{ background: 'rgba(15,143,82,0.10)', border: '1px solid rgba(15,143,82,0.28)' }}>
                <div style={{ width: '10px', height: '10px', borderRadius: '3px', background: 'var(--event-primary)', flexShrink: 0 }} />
                <div>
                  <h4 className="font-heading font-bold uppercase tracking-wider mb-1" style={{ fontSize: '0.9rem', color: '#1a1a1a' }}>{theme.title}</h4>
                  <p className="font-body" style={{ fontSize: '0.9rem', color: '#444', lineHeight: 1.45 }}>{theme.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: 'linear-gradient(180deg, #031a10 0%, #06351f 50%, #031a10 100%)', padding: 'clamp(140px,14vw,180px) clamp(24px,5vw,80px) clamp(60px,8vw,100px)' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
          <h2 className="font-heading font-bold text-white text-2xl mb-8">Why Attend Soccerex Middle East?</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid var(--event-primary-border)', borderRadius: '14px', padding: 'clamp(24px, 3vw, 36px)' }}>
              <h3 className="font-heading font-bold text-white text-xl mb-2 flex items-center gap-2">
                <Mail size={18} style={{ color: 'var(--event-primary-light)' }} /> Request Access
              </h3>
            ...
            <div style={{ background: 'linear-gradient(145deg, var(--event-primary-bg), rgba(255,255,255,0.02))', border: '1px solid var(--event-primary-border)', borderRadius: '14px', padding: 'clamp(24px, 3vw, 36px)' }}>
              ...
              <p className="font-body text-white/70 text-sm mb-5 leading-relaxed">
                Clubs, leagues, federations, national teams, competitions, and qualifying rightsholders may apply for complimentary access to Soccerex Middle East.
              </p>
              <p className="font-heading font-bold text-white text-sm mb-3">What You Unlock:</p>
              ...
              <InquiryModalButton
                kind="preregister"
                label="Apply for Rightsholder Access"
                modalTitle="Apply for Rightsholder Access"
                eyebrow="Complimentary pass"
                intro="Tell us about your organisation so the Soccerex team can review rightsholder eligibility for Soccerex Middle East."
                schema={RIGHTSHOLDER_SCHEMA}
                extraPayload={{
                  event_slug: 'soccerex-mena-2027',
                  attendee_type: 'rights_holder',
                  interest: 'Complimentary rightsholder access',
                  source: 'riyadh-rightsholder-access',
                  marketing_opt_in: true,
                }}
